class AlertModal {
  ...
  confirmationAlert({
          ...
          <div class="modal-dialog modal-${modal_size}" role="document">
            <div class="modal-content">
              <div class="modal-header" style="border-bottom: none;">
                ${
                  close
                    ? `<button type="button" class="close modal-close-btn" id="close_${id}" aria-label="Close"><i class="fa-solid fa-xmark"></i></button>`
                    : ""
                }
              </div>
    ...
    $(`#ok_${id}`)
      .off("click")
      .on("click", () => closeModalAndExecuteCallback(ok_callback));
    $(`#cancel_${id}`)
      .off("click")
      .on("click", () => closeModalAndExecuteCallback(cancel_callback));
    $(`#close_${id}`)
      .off("click")
      .on("click", () => closeModalAndExecuteCallback(close_callback));
    if (extra_btn) {
      $(`#extra_${id}`)
        .off("click")
        .on("click", () => closeModalAndExecuteCallback(extra_callback));
    }
